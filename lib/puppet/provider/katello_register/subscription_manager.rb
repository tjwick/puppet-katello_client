Puppet::Type.type(:katello_register).provide(:subscription_manager) do
  ...
  def config_parameters
    params = []
    params << "config"
    params << "--server.hostname" << @resource[:katello_hostname] if ! @resource[:server_hostname].nil?
    params << "--rhsm.repo_ca_cert" << @resource[:katello_cert] if ! @resource[:rhsm_cacert].nil?
